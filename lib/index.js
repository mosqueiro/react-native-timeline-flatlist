          ...
          {[25, 26].includes(rowData.type) ? (
            ...
            <React.Fragment>
              <Text style={[this.props.darkMode ? stylesDark.value : styles.value]}>
                ...
                {[30].includes(rowData.type) ? `${rowData.nfts_amount} ${rowData.nfts_amount > 1 ? "NFTs" : "NFT"}` : rowData.type === 14 ? this.props.translate.t("physicalCard") : this.normalizeTxValue(
                  [21, 29].includes(rowData.type) ? Math.abs(rowData.deposit_amount) : [22].includes(rowData.type) ? Math.abs(rowData.receive_amount) : [25, 26, 27].includes(rowData.type) ? (Math.abs(rowData.amount) * rowData.prices.zefi) : rowData.type === 19 ? Math.abs(rowData.amount) : rowData.type === 1 ? rowData.amount : [6, 17].includes(rowData.type) ? rowData.type === 6 && rowData.stage === 2 && !rowData.is_all ? Math.abs(rowData.rewards) : Math.abs(rowData.withdrawal_amount) : [2, 8, 15].includes(rowData.type) ? Math.abs(rowData.amount) - rowData.fee : [35].includes(rowData.type) ? Math.abs(rowData.amount2) - rowData.fee2 : [5].includes(rowData.type) && rowData.stage === 2 ? Math.abs(rowData.deposit_amount) : Math.abs(rowData.amount)
                )}{" "}
                {[30].includes(rowData.type) ? "" : [25, 26, 27].includes(rowData.type) ? "ZEFI" : [19, 21, 23, 24, 29].includes(rowData.type) ? "BRL" : rowData.type === 14 ? "" : [9, 11, 13, 15].includes(rowData.type) || rowData.transaction_type === "chargeback" ? "BRL" : rowData.type === 6 && rowData.stage === 2 && !rowData.is_all ? "zvZEFI" : rowData.stage === 2 || rowData.type === 17 ? rowData.coin.symbol2 : rowData.coin.symbol}
  ...
  _renderCircle(rowData, rowID) {
    ...
    switch (this.props.innerCircle) {
      case "icon":
        ...
        } else if ([29, 30].includes(rowData.type)) {
          innerCircle = (
