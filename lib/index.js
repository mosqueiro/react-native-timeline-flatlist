        ...
        <View>
          ...
          <Text style={[this.props.darkMode ? stylesDark.who : styles.who]}>
            {rowData.type === 23
              ...
              : [2, 5, 6, 16, 17].includes(rowData.type)
              ? rowData.to_address
              : rowData.type === 3
          ...
          {[25, 26].includes(rowData.type) ? (
            ...
            <React.Fragment>
              <Text style={[this.props.darkMode ? stylesDark.value : styles.value]}>
                ...
                {rowData.type === 14 ? this.props.translate.t("physicalCard") : this.normalizeTxValue(
                  [21].includes(rowData.type) ? Math.abs(rowData.deposit_amount) : [22].includes(rowData.type) ? Math.abs(rowData.receive_amount) : [25, 26].includes(rowData.type) ? (Math.abs(rowData.amount) * rowData.prices.zefi) : rowData.type === 19 ? Math.abs(rowData.amount) : rowData.type === 1 ? rowData.amount : [6, 17].includes(rowData.type) ? rowData.type === 6 && rowData.stage === 2 && !rowData.is_all ? Math.abs(rowData.rewards) : Math.abs(rowData.withdrawal_amount) : [2, 8, 15].includes(rowData.type) ? Math.abs(rowData.amount) - rowData.fee : [5].includes(rowData.type) && rowData.stage === 2 ? Math.abs(rowData.deposit_amount) : Math.abs(rowData.amount)
                )}{" "}
                {[25, 26].includes(rowData.type) ? "ZEFI" : [19, 21, 23, 24].includes(rowData.type) ? "BRL" : rowData.type === 14 ? "" : [9, 11, 13, 15].includes(rowData.type) || rowData.transaction_type === "chargeback" ? "BRL" : rowData.type === 6 && rowData.stage === 2 && !rowData.is_all ? "zvZEFI" : rowData.stage === 2 || rowData.type === 17 ? rowData.coin.symbol2 : rowData.coin.symbol}
                
                {[21, 22].includes(rowData.type) ? " " : ""}
                {[21].includes(rowData.type) ? rowData.coin_receive.is_token ? `(${this.normalizeTxValue(Math.abs(rowData.receive_amount))} ${rowData.coin_receive.symbol})` : `(${this.props.translate.t("defineBNB")})` : [22].includes(rowData.type) ? `(${this.normalizeTxValue(Math.abs(rowData.receive_amount))} ${"BRL"})` : ""}
                {/* {[25, 26].includes(rowData.type) ? " - " : ""}
                {[25, 26].includes(rowData.type) ? (this.normalizeTxValue(Math.abs(rowData.amount) * rowData.prices.zefi)) : ""}
                {[25, 26].includes(rowData.type) ? "ZEFI" : ""} */}
                {/* ["RAS", "LQX"].includes(rowData.coin.symbol) */}
  ...
  _renderCircle(rowData, rowID) {
    ...
    switch (this.props.innerCircle) {
      case "icon":
        ...
        } else if ([2, 3, 22, 26].includes(rowData.type)) {
          innerCircle = (
